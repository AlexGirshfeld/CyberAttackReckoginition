...
def run_validation_on_external_data(numOfapks, classifier):
    ...
    for i in range(numOfapks):
        ...
        try:
            ...
            fullarray = np.array(list(fv.values()),dtype='float64')
    ...
    pd.DataFrame(wrongApks).to_csv('mishitsAps.csv', index=False)
...
missedApks = pd.read_csv('mishitsAps.csv').values.tolist()


misseddf = mt.createDataFrame(missedApksList, list(data.columns))
apkHashes = pd.read_csv('hashes.csv').values.tolist()
apkHashesList =[]
for a in apkHashes:
    apkHashesList.append(a[0])

ourModel= ModelTrainer(apkHashesList, list(data.columns))